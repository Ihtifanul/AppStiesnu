...
const AdminEditEventScreen = ({ routeParams, onNavigate }) => {
  const eventData = routeParams?.eventData || {};
  const { themeColors } = useAppContext();
  
  const [namaEvent, setNamaEvent] = useState(eventData.subject || '');
  const [deskripsi, setDeskripsi] = useState(eventData.room === 'Kampus' ? '' : eventData.room || '');
  const [warnaEvent, setWarnaEvent] = useState(eventData.color || colors.primary);
  const [date, setDate] = useState(eventData.date ? new Date(eventData.date) : new Date());
  
  // Asumsi api tidak membawa 'pengulangan' pada get /kalender, biarkan 'sekali' sebagai fallback. 
  // Jika API memunculkan, gunakan di sini.
  const [pengulangan, setPengulangan] = useState(eventData.pengulangan || 'sekali');

  const [showDatePicker, setShowDatePicker] = useState(false);
  const [showTimePicker, setShowTimePicker] = useState(false);
  const [loading, setLoading] = useState(false);
  const [tandaiTanggal, setTandaiTanggal] = useState(warnaEvent !== colors.primary);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.put(`/kalender/${eventData.id}`, {
      ...
      if (res.data?.success) {
        Alert.alert('Sukses', 'Event berhasil diubah!', [
          { text: 'OK', onPress: () => onNavigate('AdminJadwal') }
        ]);
      } else {
        Alert.alert('Gagal', 'Tidak berhasil mengubah event.');
      }
    } catch (e) {
