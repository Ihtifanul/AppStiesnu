...
const AdminEditBeritaScreen = ({ routeParams, onNavigate }) => {
  const beritaData = routeParams?.beritaData || {};
  const { themeColors, user } = useAppContext();
  const returnRoute = user.role === 'admin' ? 'AdminBerita' : 'Berita';
  
  const [judul, setJudul] = useState(beritaData.judul || '');
  const [linkUrl, setLinkUrl] = useState(beritaData.link_url || '');
  
  const [gambarUrl, setGambarUrl] = useState(beritaData.gambar || '');
  ...
  const handleSave = async () => {
    if (!judul || !linkUrl) {
      Alert.alert('Error', 'Judul dan URL Berita wajib diisi');
      return;
    }
    ...
    try {
      ...
      const res = await api.put(`/berita/${beritaData.id_berita}`, payload);
      ...
      if (res.data?.success) {
        ...
        Alert.alert('Gagal', 'Tidak berhasil mengubah berita.');
      }
    } catch (e) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Berita</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.formCard, { backgroundColor: themeColors.cardBg }]}>
          ...
          <Text style={[styles.label, { color: themeColors.text }]}>Judul Berita</Text>
          <TextInput 
          ...
          <Text style={[styles.label, { color: themeColors.text }]}>URL Tautan Berita</Text>
          <TextInput 
            style={[styles.input, { backgroundColor: themeColors.cardBg, color: themeColors.text }]} 
          ...
          <Text style={[styles.sectionTitle, { marginTop: 10, color: themeColors.text }]}>Pratinjau</Text>
          <View style={[styles.previewCard, { backgroundColor: themeColors.background }]}>
             <Image source={{ uri: currentPreview || 'https://picsum.photos/400/200' }} style={styles.previewImg} />
             <Text style={[styles.previewTitle, { color: themeColors.text }]} numberOfLines={3}>{judul || 'Judul Kosong'}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, fontWeight: '800', color: colors.gray800, marginBottom: 12 },
  ...
  previewCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 12, padding: 12, overflow: 'hidden' },
  previewImg: { width: 80, height: 60, borderRadius: 6, marginRight: 12, backgroundColor: colors.gray200 },
  previewTitle: { flex: 1, fontSize: 12, fontWeight: '700' }
});

export default AdminEditBeritaScreen;
