...
const AdminJadwalScreen = ({ onNavigate }) => {
  ...
  const fetchJadwal = async () => {
    try {
      ...
      // Admin/Staff fetching personal jadwals too (optional but requested to mirror Users)
  ...
  const handleLongPress = (item) => {
    if (item.typeId === 'kalender') {
      Alert.alert("Aksi Event", "Pilih aksi untuk Kalender event ini:", [
        { text: "Batal", style: 'cancel' },
        { text: "Edit", onPress: () => onNavigate('AdminEditEvent', { eventData: item }) },
        {
          text: "Hapus", style: 'destructive', onPress: async () => {
            try {
              const res = await api.delete(`/kalender/${item.id}`);
              if (res.data?.success) fetchJadwal();
            } catch (e) {
              Alert.alert("Gagal", "Tidak dapat menghapus event");
            }
          }
        }
      ]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => onNavigate(user?.role === 'admin' ? 'AdminHome' : 'HomeCenter')} style={styles.backBtn}>
          <Ionicons name="arrow-back" size={24} color={colors.white} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>Manajemen Kalender</Text>
        <TouchableOpacity onPress={() => onNavigate('AdminAddEvent')} style={styles.backBtn}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.agendaSection}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <Text style={[styles.sectionTitle, { color: themeColors.text }]}>Kegiatan Terjadwal</Text>
          </View>
          ...
          {loading ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyText, { color: themeColors.textMuted }]}>Tidak ada kegiatan / event.</Text>
            </View>
          )}
        </View>
